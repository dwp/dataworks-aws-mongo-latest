...
resource "aws_s3_bucket_object" "libgplcompression_so_0" {
  ...
  content = file("${path.module}/files/llap/encryption/native/libgplcompression.so.0")
}
...
resource "aws_s3_bucket_object" "libgplcompression_so_0_0_0" {
  ...
  content = file("${path.module}/files/llap/encryption/native/libgplcompression.so.0.0.0")
}
