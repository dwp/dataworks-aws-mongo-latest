...
(
    ...
    function log_wrapper_message() {
        log_mongo_latest_message "$${1}" "status_metrics.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }
  ...
  push_metric() {
    ...
    cat << EOF | curl --silent --output /dev/null --show-error --fail --data-binary @- "http://${mongo_latest_pushgateway_hostname}:9091/metrics/job/mongo_latest"
                mongo_latest_status{snapshot_type="$SNAPSHOT_TYPE", export_date="$EXPORT_DATE", cluster_id="$CLUSTER_ID", component="MONGO_LATEST" correlation_id="$CORRELATION_ID"} $1
EOF
...
) >> /var/log/mongo_latest/status_metrics_sh.log 2>&1
